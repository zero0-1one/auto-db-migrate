...
module.exports = {
  ...
  parseCreateSqlColumn(sql, lastColumn) {
    ...
    if (util.findByQuote(sql, /\bAUTO_INCREMENT\b/i, { type: 'out' }).length > 0) {
  ...
  getCreateRowType(row) {
    let rt = util.findByQuote(row, /\bKEY\b/, { type: 'out' })
    if (rt.length > 0) {
      let rt2 = util.findByQuote(row, /\bFOREIGN KEY\b/, { type: 'out' })
