...
module.exports = {
  ...
  parseCreateSqlKey(sql) {
    ...
    if (!info) throw new Error(`暂未支持的key, 或非规范语法,请参考 SHOW CREATE TABLE.\n"${sql}"`)
    let columns = info.columns.split(',')
    info.columns = columns.map((col, i) => {
      col = col.trim()
      if (i == columns.length - 1) { //最后一个会出现排序
        if (col.toUpperCase().endsWith('DESC')) {
          info.isDesc = true
          col = col.slice(0, -4).trim()
        }
      }
  ...
  parseCreateSqlColumn(sql, lastColumn) {
    ...
    if (comment.length > 0) {
      ...
      defi = sql.slice(results[1].length + 3, start + comment[0].index)
    } else {
      defi = sql.slice(results[1].length + 3)
    }
    info.defi = defi //不包含 comment
