...
module.exports = class Migration {
  constructor(options) {
    ...
    this.sync = new AutoSync({
      simiThreshold: this.options.simiThreshold,
      maxTry: this.options.autoSyncMaxTry,
      tableFilter: [this.tableName(), ...this.options.tableFilter],
    })
  }
  ...
  tableName() {
    return '__auto_db_migrate_logs__'
  }
  ...
  //加锁 防止多进程部署时候同时执行，（升级过程中可能临时删除外键约束， 期间有其他修改将导致约束失效）
  async lock(cb) {
    ...
    try {
      await Db.transaction(this.options.db, async lockDb => {
        ...
        await db.query(`INSERT INTO ${this.tableName()}(id, version, seqName) VALUES(?,?,?) ON DUPLICATE KEY UPDATE updateAt = NOW()`, [0, '0', '0'])
  ...
  async _process(isAutoSync) {
    return await this.lock(async (db, tempDb) => {
      ...
      return count
    }, true) //锁住所有表 （升级过程中可能临时删除外键约束， 期间有其他修改将导致约束失效）
  }
  ...
  async _upgrade(db, tempDb) {
    ...
    if (lastLog && lastLog.status == -1) { //执行后签名不一致
      throw new Error(`!! 上次更新后签名不一致: ${version}(${seqName}), 需要人工校验和手动处理签名`)
    }

    let id = lastLog ? lastLog.id : 1
    if (lastLog && lastLog.status == 1) id += 1 //成功 或 执行后 sign 不对(因为已经执行了不重复执行).
    ...
    for (const { version, seqName, up, comment, sign } of upgradeData) {
      ...
      if (!sign) {
        throw new Error(`${tableName} 的签名数据丢失`)
      }
      await this._doUpgrade(db, tableName, id, version, seqName, up, comment, sign)
      id++
    }
    ...
    // }
    return id - (lastLog ? lastLog.id : 1)
  }

  async _saveBackPoint(db, createTables, sign, lastUpgrade = {}) {
    let rt = await db.query(`UPDATE ${this.tableName()} SET backPoint = ?, sign = ? 
      WHERE version = ? and seqName = ? and (sign = ? or sign = '')`,
      [
        JSON.stringify(createTables),
        sign,
        lastUpgrade.version || '0',
        lastUpgrade.seqName || '0',
        sign,
      ]
    )
    if (rt.affectedRows == 0) {
      throw new Error('Failed to save rollback point !')
    }
  }

  async _getBackPoint(db) {
    let rt = await db.query(`SELECT sign, backPoint FROM ${this.tableName()} ORDER BY id DESC LIMIT 1`)
    if (rt.length > 0 && rt[0].backPoint)
      return {
        sign: rt[0].sign,
        createTables: JSON.parse(rt[0].backPoint),
      }
  }
  ...
  async _autoSync(db, tempDb) {
    if (this.options.env != 'development') return false
    let tableName = this.tableName()
    let { lastLog } = await this._getUpgradeLogs(db, tableName)
    if (lastLog && lastLog.status != 1) {
      if (lastLog.status == -1) {
        this.warn('!! 上次升级后签名不一致,  请人工修复')
      } else {
        this.warn('上次升级存在错误, 无法进行自动同步')
      }
      return false
    }
    ...
    //计算完整的迁移算法
    let backPoint = await this._getBackPoint(db)
    let upgradeData = this.requireNoCache(this.autoSyncFilePath('autoUpgrade'))
    if (!upgradeData || upgradeData[0].sign.begin != backPoint.sign || upgradeData[0].sign.end != sign) {
    ...
    for (const tableName in createTables) {
      ...
      for (const colName in info.columns) {
        ...
        if (
          ...
          util.findByQuote(col.sql, /\bDEFAULT NULL\b/ig, { type: 'out' }).length > 0
        )
          continue
  ...
  async _getUpgradeLogs(db, tableName) {
    let rt = await db.query(`SELECT id, version, seqName, status, detail, sign FROM ${tableName} WHERE id != 0 ORDER BY id`)
    let logs = {}
    for (const row of rt) {
      let v = row.version
      if (!logs[v]) logs[v] = {}
  ...
  async _doUpgrade(db, tableName, id, version, seqName, up, comment, sign = {}) {
    this.info(`正在执行版本升级 ${version}(${seqName}) 中...`)
    let insertRt = await db.query(`INSERT IGNORE INTO ${tableName}(id, version, seqName, detail, comment) VALUES(?,?,?,?,?)`,
      [id, version, seqName, up.toString(), comment]
    )
    if (insertRt.affectedRows == 0) {
      let updateRt = await db.query(`UPDATE ${tableName} SET detail = ?, comment = ?, status = 0, backPoint = NULL, error = NULL, sign = '' 
        WHERE id = ? AND version = ? AND seqName = ? `,
        [up.toString(), comment, id, version, seqName,]
      )
      if (updateRt.affectedRows == 0) {
        throw new Error(`Update logs failed, maybe the id does not match the version: version = ${version}(${seqName}), id = ${id}`)
      }
    }
    ...
    try {
      if (sign.begin) {
        ...
        if (sign.begin != curSign) {
          ...
          let backPointSign = ''
          if (backPoint) {
            diffData = this.sync.diffByCreateTables(curTables, backPoint.createTables)
            backPointSign = backPoint.sign
          }
          throw new Error(
            `执行前签名不一致，
            期望签名：'${sign.begin}'
            实际签名：'${curSign}'
            BackPoint签名：'${backPointSign}'
            当前与BackPoint结构差异： ${jsStringify.stringify(diffData, null, 2)}`
          )
        }
      }
      if (typeof up == 'function') {
      ...
      if (sign.end) {
        ...
        if (sign.end != curSign) {
          await db.query(`UPDATE ${tableName} SET status = -1, sign = ? WHERE version =? AND seqName = ?`, [curSign, version, seqName])
          throw new Error(`执行后签名不一致， 期望签名：'${sign.end}'   实际签名：'${curSign}'`)
        }
      }
      ...
      e.message = `版本升级失败！！ ${version}(${seqName})  msg:${e.toString()} 
      upgrade: ${up.toString()}`

      throw e
    }
    this.info(`版本升级 ${version}(${seqName}) 完成。`)
  }
  ...
  async createUpgradeTable(db) {
    await db.query(
      `CREATE TABLE IF NOT EXISTS ${this.tableName()} (
        id bigint unsigned NOT NULL COMMENT 'id 0 is lock row',
        version varchar(255) NOT NULL,
        seqName char(6) NOT NULL,
        comment varchar(255) NOT NULL DEFAULT '',
        createAt datetime NOT NULL DEFAULT NOW(),
        updateAt datetime NOT NULL DEFAULT NOW() ON UPDATE NOW(),
        status tinyint NOT NULL DEFAULT 0 COMMENT '0:unfinished 1：finished  -1:sign error after upgrade',
        detail mediumtext DEFAULT NULL,
        backPoint mediumtext DEFAULT NULL,
        error mediumtext,
