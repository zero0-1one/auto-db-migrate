...
module.exports = class Migration {
  ...
  async upgrade() {
    await this.lock(async (db, tempDb) => {
      this.log(`数据库升级中...`)
      ...
      if (autoSync == 'auto') await this._autoSyncBack(db, tempDb)//执行新的 upgrade 前需要回退本地 autoSync 
  ...
  async _saveBackPoint(db) {
    ...
    await db.query(`REPLACE INTO ${this.tableName('back_point')}(id, sign, createTables) VALUES(1, ?, ?)`,
      [sign, JSON.stringify(createTables)]
    )
  }
  ...
  //仅仅回退 autoSync 部分
  async autoSyncBack() {
    await this.lock((db, tempDb) => this._autoSyncBack(db, tempDb))
  }
  ...
  async _autoSyncBack(db, tempDb) {
    ...
    if (!backPoint) return
    await this._doSync(db, tempDb, backPoint.createTables, 'autoSync')
  }

  async autoSync() {
    await this.lock((db, tempDb) => this._autoSync(db, tempDb))
  }
  ...
  async _getMigrationData(tempDb, curTables, tgtTables, name) {
    ...
    if (data && data.sign.begin == beginSign && data.sign.end == endSign) {
      ...
      await this.sync.initTempDbByTables(tempDb, curTables)
      let sign = await this.sync.doMigration(tempDb, data.up)
      if (sign != endSign) throw new Error(`执行 ${filePath} 后未能达到目标状态! 需手动修改或删除重新计算`)
      return data
    }
    //需要重新计算
  ...
  async format() {
    await this.lock((db, tempDb) => this._format(tempDb))
  }
  ...
  writeAutoSyncFile(migration, succeed, diffData, sign, name) {
    let filePath = this.autoSyncFile(name)
    let data = {
      confirm: false,
      sign: migration.sign,
