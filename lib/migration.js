...
module.exports = class Migration {
  ...
  outDir() {
    ...
    if (this.options.gitignore && !fs.existsSync(dir)) {
  ...
  async _autoSync(db, tempDb) {
    ...
    let tgtTables = await this.sync.getCreateTablesByDb(tempDb)
  ...
  async _doSync(db, tempDb, curTables, tgtTables, name) {
    ...
    if (this.options.autoSync == 'auto' || data.confirm) {
      ...
      this.log(`当前 autoSync 为 'manual'， 需要手动将 ${this.autoSyncFilePath(name)} 文件中 confirm  改为 true`)
    }
  }
