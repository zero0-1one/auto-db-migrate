...
module.exports = class Migration {
  ...
  async _upgrade(db, tempDb) {
    ...
    if (
      lastLog &&
      lastLog.version == lastUpgrade.version &&
      lastLog.status == 1 &&
      lastLog.seqName == lastUpgrade.seqName
    ) {
      return 0
    }
    if (lastLog.status == 1) await this._autoSyncBack(db, tempDb) //执行新的 upgrade 前需要回退本地 autoSync (如果上次异常则无需回退)
    let count = 0
  ...
  async _saveBackPoint(db, createTables, sign, lastUpgrade = {}) {
    ...
    await db.query(
      `REPLACE INTO ${this.tableName('back_point')}(id, version, seqName, sign, createTables) VALUES(1,?,?,?,?)`,
  ...
  async _doSync(db, tempDb, curTables, tgtTables, name) {
    ...
    if (this.options.autoSync == 'auto' || data.confirm) {
      ...
      if (risk > this.options.maxRisk && !data.confirm) {
        throw new Error(
          `当前${name}操纵风险值过高(${risk})， 请前往 ${this.autoSyncFilePath(name)} 确认, 并将 confirm  改为 true`
        )
      }
      this.info(`开始执行 ${name} ...`)
      ...
      if (sign != data.sign.end)
        throw new Error(`执行 ${name} 后未能达到目标状态 ！！\n` + JSON.stringify(diffData, null, 2))
  ...
  _dumpOldTables(oldTables, sign) {
    ...
    let content =
      '# 此文件是因为第一次使用 auto_db_migrate 时 sql 目录为空, 但数据库不为空. 自动导出现有表结构, 作为初始状态.\n'
    ...
    try {
      if (sign.begin) {
        ...
        if (sign.begin != curSign)
          throw new Error(`执行前签名不一致， 期望签名：'${sign.begin}'   实际签名：'${curSign}'`)
      }
      if (typeof up == 'function') {
      ...
      await db.query(`UPDATE ${tableName} SET error = ? WHERE version =? AND seqName = ?`, [
        e.toString(),
        version,
        seqName,
      ])
