...
class Upgrade {
   ...
   * */
  async dumpStructure(dbName, dir, { group, formatRules } = {}, prefix = '') {
    tools.setFormatRules(formatRules)
  ...
  async getStructsDiff(dbName, dir, prefix = '', { diffFilter, formatRules } = {}) {
    tools.setFormatRules(formatRules)
    let oldData = this.loadTablesFromDir(dir, prefix)
    let structs = await this.getTableStructs(dbName)
    diffFilter = diffFilter || []
    for (const tableName of diffFilter) {
      delete oldData[tableName]
      delete structs[tableName]
    }
    let contents = ''
    for (const tableName in structs) {
    ...
    return this.getDataDiff(oldData, newData)
  }
    ...
    await theone.Db.transaction(async  db => {
      await db.query(
        `CREATE TABLE IF NOT EXISTS ${tableName} (
          ...
          u_sDetail varchar(10000) NOT NULL DEFAULT '',
          u_sError varchar(10000) NOT NULL DEFAULT '',
          u_sSign varchar(255) NOT NULL DEFAULT '',
          PRIMARY KEY (u_uId),
  ...
  async doUpgrade(tableName, version, options, name, func) {
    await theone.Db.transaction(async (db) => {
      await db.execute(
        `REPLACE INTO ${tableName}(u_sVersion, u_sName, u_sDetail) VALUES(?,?,?)`, [version, name, func.toString()]
      )
    }, options)
  ...
  loadTablesFromDir(dir, prefix) {
    ...
    for (const file of files) {
      ...
      if (stat.isFile()) {
        ...
        let data = this.loadTablesFromString(content)
        for (let tableName in data) {
          if (all[tableName]) throw new Error(`重复的 table: ${tableName}  file:${file}`)
          all[tableName] = data[tableName]
        }
      }
    }
    return all
  ...
  loadTablesFromString(content) {
    ...
    let tables = tools.splitOutString(content, ';')
  ...
  getDataDiff(lhs, rhs) {
    ...
    return { tableCount, rowCount, diffData, info, upgrade: [] }
  }
}
