...
module.exports = class autoSync {
  ...
  delRefForeignKeysByCol(foreignKeys, tableName, colName, outMigration) {
    for (const fkInfo of foreignKeys) {
      if (fkInfo.isDrop) continue
      if (
        ...
        fkInfo.isDrop = true
        this.getAlterSql_delKey(fkInfo.selfTableName, fkInfo, outMigration)
      }
    }
  }

  //通过key 查找与其相关的 外键  (有可能自身是自身的外键，)
  delRefForeignKeysByKey(foreignKeys, tableName, keyColumns, outMigration) {
    for (const fkInfo of foreignKeys) {
      if (fkInfo.isDrop) continue
      if (
        (fkInfo.selfTableName == tableName && util.deepEqual(fkInfo.columns, keyColumns)) ||
        (fkInfo.tableName == tableName && util.deepEqual(fkInfo.refColumns, keyColumns))
      ) {
        fkInfo.isDrop = true
        this.getAlterSql_delKey(fkInfo.selfTableName, fkInfo, outMigration)
      }
  ...
  getColumnMigration({ curTables, tgtTables }, outMigration) {
    let { changeTables } = this.diffByCreateTables(curTables, tgtTables)
    let foreignKeys = this.getAllForeignKeys(curTables)
    for (const { cur, tgt } of changeTables) {
      let curInfo = sqlParser.parseCreateSql(cur.sql, ['columns'])
      let tgtInfo = sqlParser.parseCreateSql(tgt.sql, ['columns'])
      this.getMigrationSql_columns(tgt.name, curInfo, tgtInfo.columns, foreignKeys, outMigration)
    }
  }
  ...
  getKeyMigration({ curTables, tgtTables }, outMigration) {
    ...
    for (const { cur, tgt } of changeTables) {
      ...
      this.getMigrationSql_keys(tgt.name, curInfo.keys, tgtInfo.keys, foreignKeys, outMigration)
    }
  }
  ...
  getMigrationSql_columns(tableName, curInfo, tgtColumns, foreignKeys, outMigration) {
    ...
    for (const colName in curColumns) {
      ...
      } else if (curColumns[colName].sql != tgtColumns[colName].sql) {
        changeColumns[colName] = tgtColumns[colName]
      }
    }
    ...
    for (const colName in delColumns) {
      this.getAlterSql_delColumn(tableName, delColumns[colName], outMigration, foreignKeys)
    ...
    for (const colName of tgtOrder) {
      ...
      if (oldName) {
        ...
        this.getAlterSql_changeColumn(tableName, info, info.pre, outMigration, foreignKeys)
    ...
    for (const colName in changeColumns) {
      let info = changeColumns[colName]
      this.getAlterSql_changeColumn(tableName, info, info.pre, outMigration, foreignKeys)
    }
  ...
  getMigrationSql_keys(tableName, curKeys, tgtKeys, foreignKeys, outMigration) {
    let addKeys = {}
    let delKeys = {}
    let changeKeys = []
    let renameKeys = {}
    ...
    for (const keyName in delKeys) {
      this.getAlterSql_delKey(tableName, delKeys[keyName], outMigration, foreignKeys)
    }
    for (const { cur, tgt } of changeKeys) {
      this.getAlterSql_delKey(tableName, cur, outMigration, foreignKeys)
    }
  ...
  getAlterSql_delKey(tableName, info, outMigration, foreignKeys) {
    if (info.type != 'foreignKey') this.delRefForeignKeysByKey(foreignKeys, tableName, info.columns, outMigration)
  ...
  getAlterSql_delColumn(tableName, info, outMigration, foreignKeys) {
    this.delRefForeignKeysByCol(foreignKeys, tableName, info.name, outMigration)
  ...
  getAlterSql_changeColumn(tableName, info, afterCol, outMigration, foreignKeys) {
    this.delRefForeignKeysByCol(foreignKeys, tableName, info.oldName || info.name, outMigration)
    let sql = ''
