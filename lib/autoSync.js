...
module.exports = class autoSync {
  ...
  async createMigrationByTables(tempDb, curTables, tgtTables) {
    ...
    let newSign = this.getTablesSign(newTgtTables)
    let results = { migration, succeed: sign.end == newSign, sign }
    if (!results.succeed) results.diffData = this.diffByCreateTables(newTgtTables, tgtTables)
    return results
  }
