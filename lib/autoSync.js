...
module.exports = class autoSync {
  constructor(options = {}) {
    ...
    this.tableFilter = new Set(options.tableFilter || [])
  }
  ...
  async createMigrationByTables(tempDb, curTables, tgtTables) {
    ...
    let migration = []
    await this._sync(tempDb, _tgtTables, 'Table', migration)
  ...
  getOptionMigration(diffData) {
    ...
    for (const { cur, tgt } of diffData.changeTables) {
      ...
      if (curInfo.options.sql != tgtInfo.options.sql) {
        migration.push(`ALTER TABLE ${ID(cur.name)} ${tgtInfo.options.sql}`)
      }
    }
    return migration
  }
  ...
  _checkTableKey(tableInfos, tableName, refColumns) {
    if (!tableInfos.hasOwnProperty(tableName)) return false
    let keys = tableInfos[tableName].keys
    for (const keyName in keys) {
      let { type, columns } = keys[keyName]
