...
module.exports = {
  ...
  getTableGroup(dir, prefix = '') {
    ...
    for (const file of files) {
      ...
      if (stat.isFile()) {
        ...
        group[groupName] = {
          tables: sqlParser.parseTableNames(content),
          content
        }
      }
    }
  ...
  async createMigration(db, tempDb, maxTry = 10) {
    ...
    let beginSign = this.getTablesSign(curTables)
    let endSign = this.getTablesSign(tgtTables)
  ...
  async doMigration(db, migration, force = false) {
    ...
    return { succeed, failed, unexecuted, msg }
  },

  async sync(db, migration, type = 'up') {
    let { failed, msg } = await this.doMigration(db, migration[type])
